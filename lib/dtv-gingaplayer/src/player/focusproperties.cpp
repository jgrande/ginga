	...
	: SurfaceProperties( canvas )
{
	...
	_selected = false;
	_applied = false;	
	_borderColor =  util::cfg::getValue<std::string>( "default.focusBorderColor" );
	_selBorderColor = util::cfg::getValue<std::string>( "default.selBorderColor" );
	_width = util::cfg::getValue<int>( "default.focusBorderWidth" );;
	_transparency = util::cfg::getValue<float>( "settings.focusBorderTransparencyAsFloat" );
}
