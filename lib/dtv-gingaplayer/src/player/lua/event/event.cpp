...
util::id::Ident Module::connect( const std::string &host, int port, util::DWORD /*timeOut*/ ) {
	...
	util::net::SockAddr remoteAddr;
	{
		util::net::ipv4::Address addr( host );
		util::net::ipv4::SockAddr sockAddr( addr, (unsigned short) port );
		remoteAddr = sockAddr;
	}

	//	Try connect to remote host
	if (!sock->connect( remoteAddr )) {
		LERROR("lua::Event::Module", "Cannot connect to host: host=%s, port=%d", host.c_str(), port );
		return socketID;
	}
