...
static int l_attrFont( lua_State *L ) {
	...
	if (lua_gettop(L) == 1) {
		...
		{	//	parse font lua style 
			...
			::canvas::Font font( face, size, bold, sty, variant );
...
static int l_drawEllipse( lua_State *L ) {
	...
	if (!strcmp( "fill", mode )) {
		wrapper->surface()->fillEllipse( point, w, h, arcW/2, arcH/2 );
	}
	else if (!strcmp( "arc", mode )) {
		wrapper->surface()->drawEllipse( point, w, h, arcW/2, arcH/2 );
	}
	else {
